...
import { useVerifyEmail } from "../03-features/auth/hook/useVerifyEmail";
 
import { toast } from 'react-toastify';
import { useDispatch, useSelector } from "react-redux";
import { clearUser } from "../03-features/auth/hook/01-useSlice";
import { useResendCode } from "../03-features/auth/hook/useResendCode";
import { ClipLoader } from "react-spinners";
import { useChangeEmailVerification } from "../03-features/auth/hook/useChangeEmailVerification";
import { useQueryClient } from "@tanstack/react-query";

export function VerifyRegisterEmail(){
    ...
    const dispatch = useDispatch();  // You can call the clearUser action when the user logs out or when verification is complete:
    ...
    const {mutate:changeEmailVerification}=useChangeEmailVerification(); // for change email verifation after login 

    const {mutate:resendCode}=useResendCode() // before login 
    ...
    // Detects whether the current user is already logged in or not , if login then use this for user change Emial verifcation else user Register email verifacton and redirect to login after verifaction
...
useEffect(() => {
    ...
    function handleSubmit(e){
        ...
        if(userDetail.userId){
            ...
            changeEmailVerification({email,code:codeString},{
            onSuccess:async ()=>{
                ...
                setCode(["", "", "", "", "", ""]);
                 ...
                 navigate("/account/profile",{replace:true}); // Redirect to login or dashboard
            },
            onError: (error) => {
        ...
        mutate({email,code:codeString},{
            onSuccess:()=>{
                ...
                setCode(["", "", "", "", "", ""]);
                 ...
                 dispatch(clearUser());
                 navigate("/login",{replace:true}); // Redirect to login or dashboard
            },
            onError: (error) => {
            setError(error.response?.data?.message || "Something went wrong")
            ...
            <section className="flex justify-center mt-15 px-2">
                 <div className="flex flex-col gap-6 w-auto  sm:w-[50%]  lg:w-[45%] xl:w-[30%]  h-auto shadow-lg rounded-lg py-4">
                    <div className="bg-blue-600 h-20 rounded-t-lg flex justify-center items-center text-white text-xl font-bold">
                             {userDetail.userId ? "Verify Changed Email" : "Verify Register Email"}
                    ...
                    <form className="px-2 sm:px-6" onSubmit={handleSubmit}>
                        <div className="flex flex-col gap-6 border-l-3 border-blue-600 bg-slate-200 font-normal my-2 p-2 rounded-lg">
                            <h1 className="font-semibold "><MdOutlineSecurity className="inline text-blue-600"/> Email Verification Method</h1>
