...
import { FaBoxes, FaPercent, FaRupeeSign  } from "react-icons/fa";
...
import { useEditProduct } from "../../../hook/03-uploadProducts/04-useEditProduct";
 
 


//  setIsOpen,ParentcategoryName,setParentCategoryName, categoryId, setCategoryId, subCategoryName, setSubCategoryName, selectedSubCategoryId ,setselectedSubCategoryId

export function FromForEditProduct({ setIsOpen ,productId, ParentcategoryName,setParentCategoryName,categoryId,setCategoryId, subCategoryName, setSubCategoryName , selectedSubCategoryId ,setselectedSubCategoryId,product,setProduct,fields,setFields,images}) {
  
  
  
  const [fileName, setFileName] = useState("");
  const [image, setImage] = useState(null);
   const [croppedImages, setCroppedImages] = useState([]); // ✅ all images (old + new)
    const [removedImages, setRemovedImages] = useState([]); // ✅ store deleted Cloudinary public_ids




    // 2. Pre-load product images from DB
    useEffect(()=>{
      console.log(images)
        if(images){
    ...
    function handleRemoveImage(idx) {
       setCroppedImages((prev)=>{
        ...
        // if it's an old DB image → mark for Cloudinary deletion
        if(!removed.isNew){
            setRemovedImages((r)=>[...r,removed.publicId])
        }
...
function handleSubmit(e) {
  ...
  console.log(productId+"ddddddddddd");
...
// add new images only
  ...
  // add deleted images public_ids
  removedImages.forEach((publicId)=>{
    formData.append("deleted_images[]", publicId);  //Each publicId is appended as a separate form entry, but all entries share the same name: deleted_images[]. ,,, On the backend (Express / multer / body-parser): It automatically collects all entries with the same name into an array.
  });
  ...
  setProcessing(true);
   mutate(formData, {
          onSuccess: async (data) => {
           ...
           setCategoryId(null); // reset category
      ...
      <form onSubmit={handleSubmit} className="my-6 flex flex-col gap-4 ">
        <div className="text-sm md:text-lg relative">
          ...
          <div className={`absolute z-10 top-17 w-full max-h-50 overflow-auto border border-t-0  shadow-md mt-2 bg-white transition-all duration-500 ease-in-out transform origin-top  ${
           ...
           <SearchOrSelect Data={AllCategories.categories}   setIsShowOptions={setIsShowParentCategory} setSelectId={ setCategoryId} setSelectIdName={setParentCategoryName} />
           </div >
        </div>
        <div className="text-sm md:text-lg relative">
           Parent Sub-Category:  
      ...
      <div className="flex justify-center my-6 relative">
         {image && (
            <div className="flex flex-col items-center gap-4 ">
              {/* Cropper */}
              <div
                className="flex flex-col items-center     overflow-hidden border rounded-lg m-4"
                 style={{ maxHeight: "80vh", width: "300px" }}
              >
                <ReactCrop
                  crop={crop}
