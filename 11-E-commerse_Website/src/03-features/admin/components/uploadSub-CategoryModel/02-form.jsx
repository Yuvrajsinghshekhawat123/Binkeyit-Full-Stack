...
import { useAddSubCategory } from "../../hook/02-subCatgory/01-useAddSubCategory";
import { usegetAllCategories } from "../../hook/01-Category/02-useGetAllCategory";

export function FromForUploadSubCategory({ setIsOpen ,subCategoryName, setSubCategoryName, ParentcategoryName,setParentCategoryName, CategoryId , selectedSubCategoryId}) {
      ...
      (blob) => {
        ...
        formData.append("categoryId", selectedCategoryId);
         ...
         // ✅ Only append image if new one is selected
          if (image) {
            formData.append("sub-category_image", croppedFile);
          }


        // ✅ Pass id only when editing
    ...
    <>
      <form onSubmit={handleSubmit} className="my-6">
        <div className="text-sm md:text-lg relative">
          ...
          {/* Trigger */}
          <div
            onClick={() => setIsShowParentCategory((prev) => !prev)}
          ...
          {/* Dropdown */}
          <ul
            className={`absolute z-10 top-17 w-full max-h-40 overflow-auto border border-t-0  shadow-md mt-2 bg-white transition-all duration-500 ease-in-out transform origin-top  ${
              isShowParentCategory
          ...
          >
            {AllCategories.categories.map((category) => (
              <li
                key={category.id}
                className="cursor-pointer p-2 border-b-1 border-gray-300 hover:bg-gray-200 "
                onClick={() => {
                  setSelectedCategoryId(category.id);
                  setParentCategoryName(category.name);
                  setIsShowParentCategory(false);
                }}
              >
                {category.name}
              </li>
        ...
        <div className="flex justify-end space-x-2">
          ...
          <button
            ...
            disabled={
              processing || !fileName || !subCategoryName || !selectedCategoryId
            }
            title={
          ...
          >
            ...
            >
              <img
                ...
                className="w-full h-full object-contain rounded-lg my-4"
